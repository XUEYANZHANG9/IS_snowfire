...
if (basin == "nwinterior") or (basin == "plains") or (basin == "coastalnorth") or (basin == "coastalsouth") or (basin == "southwest"):
	sm_swe_mask = np.ma.logical_and(hist_swe,sm) 
	...
	sm_masked_res = sm_masked.reshape(3,a,b,c) 
	sm_hist_masked_res = sm_hist_masked.reshape(3,56,b,c) 
	sm_hist_masked_mean = np.ma.apply_along_axis(np.mean,0,sm_hist_masked_res) 
	sm_masked_mean = np.ma.apply_along_axis(np.mean,0,sm_masked_res)
	## subtract minimum historical soil moisture in each grid cell from each summer soil moisture average  
	sm_minstorage = np.ma.apply_along_axis(subtract_min_sm,0,sm_masked_mean,sm_hist_masked_mean)  
	## get cell areas 
