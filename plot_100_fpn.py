...
for ii, f in enumerate(txt_files):
	...
	v_dfm_low = v.where(mask_domain_lowlands == 1) 
        dfm_vals_m = v_dfm_mtns.values
	dfm_vals_l = v_dfm_low.values
        dfm_vals_m[dfm_vals_m < 0] = -1.00
	dfm_vals_l[dfm_vals_l < 0] = -1.00
        dfm_vals_m[dfm_vals_m > 0] = 1.00
	dfm_vals_l[dfm_vals_l < 0] = 1.00 
        if ii == 0:
        	fpn_sum_m = dfm_vals_m
		fpn_sum_l = dfm_vals_l
        else:
        	fpn_sum_m += dfm_vals_m
		fpn_sum_l += dfm_vals_l
...
fpn_m = fpn_sum_m / total
fpn_l = fpn_sum_l / total

fpn_mountains = fpn_m
fpn_lowlands = fpn_l

fpn_mountains = fpn_mountains * 100.0
 
v_dfm_mtns.values = fpn_mountains
vmin=-100
vmax=100 
'''
vmin = v_dfm_mtns.min()
...
fpn_lowlands = fpn_lowlands * 100.0

v_dfm_low.values = fpn_lowlands
'''
vmin = v_dfm_low.min()
vmax = v_dfm_low.max()
'''
